...
app.get('/api/health',c=>c.json({ok:true,version:'1.0.0',persistence:c.env.DB?'d1':'memory-fallback',ai:createAIProvider(c.env)?.name||'not-configured',structuredMemory:true,checkpoints:true}))
...
app.post('/api/ai/chat',async c=>{const denied=requireAuth(c);if(denied)return denied;const b=await json(c),message=typeof b?.message==='string'?b.message.trim().slice(0,20000):'';if(!message)return c.json({error:'Message is required'},400);const provider=createAIProvider(c.env);if(!provider)return c.json({error:'AI provider is not configured'},503);const pkg=await contextFor(c,message,true,bool(b.includePrivate));try{const answer=await provider.chat([{role:'system',content:`You are connected to Memory.Nrul. Treat memory as context, not unquestionable truth. Current user input overrides stale memory. Never claim access to excluded data.\n\n${formatSystemContext(pkg)}`},{role:'user',content:message}]);return c.json({ok:true,provider:provider.name,model:provider.model,answer,contextItems:pkg.context.length,contextVersion:pkg.version,privacy:{filterApplied:true,privateIncluded:pkg.privacy.privateIncluded,restrictedBlocked:true}})}catch{return c.json({error:'AI provider request failed'},502)}})
