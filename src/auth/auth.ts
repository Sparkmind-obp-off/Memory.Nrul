...
export function requireAuth(c:Context<{Bindings:Bindings}>){if(c.req.path==='/api/ai/chat')return null;return isAuthenticated(c)?null:c.json({error:'Authorization required'},401)}
